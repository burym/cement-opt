(function($) {
	...
	$.cartonbox = function(options) {
		...
		$('.cartonbox-wrap').on('click', '.cartonbox-left a, .cartonbox-right a, .cartonbox-nav li:not(.active) a', function() {
			if ($(this).parent().hasClass('cartonbox-left')) settings.onLeft($(this));
			if ($(this).parent().hasClass('cartonbox-right')) settings.onRight($(this));
			...
			if ($('.cartonbox-on').length) {
				if (e.which == 37) { // left
					if ($('.cartonbox-left a').length) {
						settings.onLeft($('.cartonbox-left a'));
						cbFuncContent($('.cartonbox-left a'));
					}
				} else if (e.which == 39) { // right
					if ($('.cartonbox-right a').length) {
						settings.onRight($('.cartonbox-right a'));
		...
		function cbFuncStart(cbThis) {
			...
			// то сохраняем его
			if (!$('.cartonbox-body').data('cb-existing-hash')) $('.cartonbox-body').data('cb-existing-hash', window.location.hash);
		...
		function cbFuncEndLoad(cbThis, cbImgW, cbImgH) {
			...
			} else settings.onShowAfter(cbThis);
		...
		function cbFuncClosed() {
			...
			if (!ie) {
				...
				if ($('.cartonbox-body').data('cb-existing-hash') != '') window.history.replaceState(null, null, $('.cartonbox-body').data('cb-existing-hash'));
			}
